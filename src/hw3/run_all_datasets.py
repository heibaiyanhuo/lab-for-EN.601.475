...
# DATASETS = [ 'hard']
# DATASETS = ['easy', 'bio', 'finance', 'speech', 'vision']
...
for dataset in DATASETS:
    ...
    unformatted_cmd = 'python classify.py --data %s --mode train --model-file %s --algorithm %s --num-boosting-iterations 10'
    ...
    for subset in ['train', 'dev', 'test']:
        ...
        unformatted_cmd = 'python classify.py --data %s --mode test --model-file %s --predictions-file %s'
        ...
        if subset != 'test':
            ...
            cmd = 'python compute_accuracy.py %s %s' % (data, predictions_file)
            os.system(cmd)